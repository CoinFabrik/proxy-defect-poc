 ...
 * returned by the abstract getImplementation() internal function.
 */
contract Proxy {
   ...
   */
  function admin() external view proxyIfNotAdmin returns (address) {
    return getAdmin();
  }
   ...
   */
  function implementation() external view proxyIfNotAdmin returns (address) {
   ...
   * It should include the signature and the parameters of the function to be called, as described in
   * https://solidity.readthedocs.io/en/v0.4.25/abi-spec.html#function-selector-and-argument-encoding.
   */
  function upgradeToAndCall(address newImplementation, bytes data) payable external proxyIfNotAdmin {
    setImplementation(newImplementation);
...
contract Test {
  ...
  function changeAdmin(address newAdmin) payable external {
    emit TestSucceeded(newAdmin, msg.value);
  }
  ...
  function upgradeTo(address newImplementation) payable external {
    emit TestSucceeded(newImplementation, msg.value);
  }
